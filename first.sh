...
# Author: norgeous
# from: https://bbs.nextthing.co/t/a-few-things-that-a-new-chip-pocketchip-owner-should-do-know-and-try/13803
# tested with 4.4 headless image
...
if (whiptail --title "Enable ll command" --yesno "Enable ll command?" 8 40) then
  ...
  # ll for user 1000
  sed -i "s/# export LS_OPTIONS='--color=auto'/export LS_OPTIONS='--color=auto -haX --group-directories-first'/g" "/home/$NEWUSER/.bashrc"
  sed -i "s/# alias ll/alias ll/g" "/home/$NEWUSER/.bashrc"
